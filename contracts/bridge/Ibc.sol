...
contract Ibc is Ownable, ReentrancyGuard {
    /*
        A constant that represents this chain as a recipient. It's the enum index of this Acurast enum:

        pub enum Subject<AccountId, Contract> {
            Acurast(Layer<AccountId, Contract>) = 0,
            AlephZero(Layer<AccountId32, Contract>) = 1,
            Vara(Layer<AccountId32, Contract>) = 2,
            Ethereum(Layer<AccountId20, AccountId20>) = 3,
            Solana(Layer<AccountId32, AccountId32>) = 4,
        }
    */
    uint8 public constant ACURAST_INDEX = 0;
    uint8 public constant ETHEREUM_INDEX = 3;
    ...
    struct OutgoingMessage {
        bytes32 id;
        // uint8 senderIndex; <- since we know senderIndex is always ETHEREUM_INDEX it's redundant to store
        address sender;
        bytes32 nonce;
    ...
    event MessageProcessedWithErrors(
        uint8 senderIndex,
        bytes32 sender,
        // uint8 recipientIndex; <- since we know recipientIndex is always ETHEREUM_INDEX it's redundant to emit in event
        address recipient,
        bytes payload,
        string reason
    );
    ...
    function confirmMessageDelivery(
        ...
        // Store fee amount and payer before deleting state
        uint256 feeAmount = outgoing[id].fee;
        address payable feeReceiver = payable(msg.sender);
    ...
    function receiveMessage(
        ...
        require(
            ...
            "Message recipientIndex invalid"
        );
    ...
    function checkSignatures(
        ...
        // For small arrays (typical signature counts), the original approach is actually fine
        // But we can optimize it by using a smaller array and early termination
        address[] memory seenSigners = new address[](min_signatures);
        ...
        for (uint256 i = 0; i < signatures.length; i++) {
            ...
            // Check for duplicates - only check against already seen signers
            ...
            // Early termination: if we have enough unique signatures, we can stop
            if (validSignatures >= min_signatures) {
                break;
            }
        }

        // SECURITY FIX: Ensure final validation always executes
        require(
